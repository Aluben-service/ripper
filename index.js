		...
		wss.on("connection", (conn) => {
			...
			blazeProxy.all("*", async (req, res) => {
				try {
					...
					if (file.status != 404 || file.status != 403) {
						if (
							!fs.existsSync(
								`./downloads${req.baseUrl + req.path}`,
							)
						) {
							fs.mkdir(
								`./downloads${url}`,
						...
						setTimeout(() => {
							if (req.path.includes(".") && req.path !== "/") {
								...
								try {
									fs.writeFileSync(
										`./downloads${req.baseUrl + "/index.html"}`,
										data,
									);
								} catch (e) {
									console.error(e);
								...
								try {
									fs.writeFileSync(
										`./downloads${req.baseUrl + req.path + "." + fileExtension}`,
										data,
									);
								} catch (e) {
									console.error(e);
			...
			blazeProxyServer.addListener("error", (e) => {
				if (e.message.includes("EADDRINUSE")) {
					...
					console.log(e);
					console.log(
						"The blaze proxy server encountered an error ^^^^^^^^^^^",
					);
					try {
						return res.json({
							error: true,
							errorMsg:
								"The blaze proxy server encountered an error. Please check the logs.",
						});
					} catch (e) {}
				}
			});
			...
			blazeProxyServer.addListener("listening", () => {
				try {
					return res.json({
						error: false,
						port: blazeProxyServer.address().port,
					});
				} catch (e) {}

				console.log(
					`Your Blaze proxy server is running on port ${blazeProxyServer.address().port}`,
				);
			});
...
app.use((req, res) => {
	...
	res.sendFile(
		path.join(
			url.fileURLToPath(new URL("./content/", import.meta.url)),
			"/404.html",
		),
	);
});
...
const blazeServer = app.listen(5000, () => {
	console.log(
		`Your Blaze server is running on port ${blazeServer.address().port} using node ${process.version}`,
	);
});
